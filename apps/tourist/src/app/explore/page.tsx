...
export default async function ExplorePage({
  ...
  const { guest, q, category } = await searchParams;
  ...
  const heritageCategorySlug =
    categories.find((item) => (item.name_i18n?.es ?? "").toLowerCase().includes("patrimonio"))?.slug ??
    categories.find((item) => item.slug.toLowerCase().includes("heritage"))?.slug ??
    null;
